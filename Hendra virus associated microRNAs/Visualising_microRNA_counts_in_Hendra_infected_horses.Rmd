...
```
## Data clean up
Plotting the total counts for each sample or library we see that they vary around the mean for this experiment, `r mean(lib_size$counts_sum)`million counts. Raw counts, which we would like to use as a measure of microRNA expression will be higher for a microRNA in a sample with higher total counts. We can therefore convert the counts to counts per million (CPM) using the edgeR package. 
```{r plotting library size, out.width = '50%', echo = FALSE, fig.align = 'left'}
microRNA_counts %>%
  gather(sample, counts, -gene) %>% 
  left_join(redlands_horse_metadata_tidy, by = "sample") %>% 
  rename(library = sample) %>% 
