...
The the `voom` weights calculated by the distance from the red curve feed into`limma` so that the variance is no longer dependent on the mean expression level. Here I used the `lmFit` function with model `~ day + animal`. Unfortunately many of the significant coefficients are close to zero meaning that the day post infection has little impact on expression of any microRNAs. 
